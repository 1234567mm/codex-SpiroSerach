...
| Ticket | Title | Description |
|--------|-------|-------------|
| T27-G1 | Integrate RDKit molecular fingerprints into Beard/Cole training pipeline | Install RDKit via `[ml]` extras. Extend `beard_cole_training.py:_features_for_record()` with `--features chemical` flag: compute Morgan fingerprints (radius=2, 2048 bits) from SMILES via RDKit, append to the 5 device-structural features. Train a `SklearnSurrogate` (GPR with Matérn 2.5 kernel) on the combined feature set. Compare RMSE vs. the 5-feature baseline. |
| T27-G2 | Replace HeuristicSurrogate with fingerprint-based GPR | In `surrogate.py`, replace `HeuristicSurrogate` (nearest-neighbor, line 318-434) as the default production model with the fingerprint-enhanced `SklearnSurrogate`. Gate the replacement behind a `model_version` bump. The old HeuristicSurrogate remains available as a baseline via `--model heuristic`. |
...
| Ticket | Title | Description |
|--------|-------|-------------|
| T27-H1 | Split monolithic JS into ES modules | Refactor `run-data-store.js` (1659 lines) and `candidate-projection.js` (1054 lines) into ES modules: `store.mjs`, `adapters.mjs`, `projections.mjs`, `viewer.mjs`. Use `import`/`export` syntax. Keep `index.html` loading via `<script type="module">`. No bundler required — native ES modules work in all modern browsers. Remove the `globalThis.SpiroRunData` IIFE pattern. |
| T27-H2 | Add virtual scrolling to candidate table | Replace the `renderCandidateWorkspace()` full-table `innerHTML` with a virtual scrolling approach: only render rows visible in the viewport + 5-row buffer. Use `IntersectionObserver` for lazy detail panel rendering. Target: <100ms render time for 200 candidates (currently ~500ms+ full re-render). |
...
| Ticket | Title | Description |
|--------|-------|-------------|
| T27-I1 | Add DevContainer + Dockerfile | Create `.devcontainer/devcontainer.json` and `Dockerfile`: Python 3.11, RDKit, xtb, Node.js 20. Works with VS Code Remote Containers and GitHub Codespaces. Document in `docs/devcontainer-setup.md`. Anyone can clone and press "Reopen in Container" to get a working environment. |
...
| Ticket | Title | Description |
|--------|-------|-------------|
| T27-J1 | Write ADR 0004 (active learning handoff) + ADR 0005 (release hardening) | **ADR 0004**: Records the V24 decision to use offline/partner-assisted handoff (not direct lab dispatch), the observation projection read model, and the stop/continue report design. **ADR 0005**: Records the V25 decision to keep optional `[ml]`/`[bo]` dependencies isolated, the migration policy of "fail closed on missing schema refs," and the deliberate choice not to claim external scientific validation. |
| T27-J2 | Write operator runbook and API reference | Create `docs/operator-runbook.md`: (1) installation from clone, (2) running a screening job, (3) viewing results in the artifact viewer, (4) backing up and restoring outputs, (5) adding a new candidate, (6) troubleshooting common errors. Create `docs/api-reference.md`: Python API for `ReadOnlyRunAPI`, `JsonArtifactRepository`, `ScreeningPolicy`, CLI commands. |
...
|----|------|-------------|--------|------------|
| R27-1 | `xtb` fails to install or compute on Windows | Medium | High | Use the RDKit-only fallback: compute molecular descriptors without DFT energies. Document this as a known limitation. |
| R27-2 | RDKit installation fails on some platforms | Medium | Medium | Feature-gate `--features chemical` behind `try: import rdkit`. The 5-feature baseline remains the default. |
| R27-3 | Fingerprint GPR does not beat the 5-feature baseline on 7 records | High | Medium | Accept the negative result. Document that more data is needed. This is scientifically honest — T27-G3 (GNN feasibility) then becomes the path forward. |
| R27-4 | GNN feasibility concludes "insufficient data" | High | Low | This is the expected outcome. The feasibility report is the deliverable, not the GNN. |
| R27-5 | ES module migration breaks the Node VM test harness | Medium | Medium | Update the Node VM test to use `vm.Module` or `import()`; if infeasible, keep Node VM tests on the pre-migration bundle and test ES modules only via Playwright. |
| R27-6 | Docker on Windows requires WSL2, which may not be available | Low | Low | DevContainer is optional; document the non-Docker setup path as primary. |
| R27-7 | Performance benchmarks fail due to CI runner variance | Medium | Low | Set generous regression thresholds (±30% instead of ±20%) and use relative comparisons within the same CI run. |
